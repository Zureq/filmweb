...
public class ImageDrawer {
	...
	public static BufferedImage createJPG(Film movie) {
		...
		Font f = new Font(g.getFont().getName(), g.getFont().getStyle(), 20);
		g.setFont(f);
		StringBuilder sb = new StringBuilder();
		sb.append(movie.getTitle());
		sb.append(" (");
		sb.append(movie.getPolishTitle());
		sb.append(")");
		g.setColor(Color.BLUE);
		g.drawString(sb.toString(), 10, 20);
		f = new Font(g.getFont().getName(), g.getFont().getStyle(), 16);
		g.setFont(f);
		g.setColor(Color.RED);
		g.drawString("Ocena: " + String.valueOf(movie.getRate()), 10, 50);
		g.setColor(Color.BLACK);
		g.drawString("Gatunek: " + movie.getGenre(), 10, 80);
		Rectangle2D rect = g.getFontMetrics().getStringBounds(
				movie.getDescription(), g);
		List<String> textList = wrap(movie.getDescription(),
				g.getFontMetrics(), 590);
		int i = 0;
		for (String text : textList) {
			g.drawString(text, 10, 100 + g.getFontMetrics().getHeight() * i);
			i++;
		}
		Image img;
		try {
			img = ImageIO.read(movie.getCoverUrl());
			g.drawImage(img, 650, 20, 400, 500, null);
		...
		try {
			ImageIO.write(
					image,
					"jpg",
					new File("c:/filmweb/obrazki/"
							+ ((movie.getTitle() != null ? movie.getTitle()
									: movie.getPolishTitle()).replaceAll(
									"[^a-zA-Z0-9.-]", " ")) + ".jpg"));
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	private static List wrap(String str, FontMetrics fm, int maxWidth) {
	 ...
	 */
	private static void wrapLineInto(String line, List list, FontMetrics fm,
			int maxWidth) {
		int len = line.length();
		int width;
	 ...
	 */
	private static List splitIntoLines(String str) {
		ArrayList strings = new ArrayList();
